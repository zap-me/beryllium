...
import payouts_core
from tasks import task_manager, send_email
...
def _fiat_deposit_email(fiat_deposit: FiatDeposit):
    if fiat_deposit.status == fiat_deposit.STATUS_COMPLETED:
        task_manager.one_off('send_email', send_email, ['Deposit Completed', _fiat_deposit_email_msg(fiat_deposit, ''), fiat_deposit.user.email])
    if fiat_deposit.status == fiat_deposit.STATUS_EXPIRED:
        task_manager.one_off('send_email', send_email, ['Deposit Expired', _fiat_deposit_email_msg(fiat_deposit, ''), fiat_deposit.user.email])
...
def _fiat_withdrawal_email(fiat_withdrawal: FiatWithdrawal):
    if fiat_withdrawal.status == fiat_withdrawal.STATUS_COMPLETED:
        task_manager.one_off('send_email', send_email, ['Withdrawal Completed', _fiat_withdrawal_email_msg(fiat_withdrawal, ''), fiat_withdrawal.user.email])
...
def _crypto_deposit_email(deposit: CryptoDeposit):
    if deposit.confirmed:
        task_manager.one_off('send_email', send_email, ['Deposit Confirmed', _crypto_deposit_email_msg(deposit, 'confirmed', ''), deposit.user.email])
    else:
        task_manager.one_off('send_email', send_email, ['Deposit Incoming', _crypto_deposit_email_msg(deposit, 'incoming', ''), deposit.user.email])
...
def _crypto_withdrawal_email(crypto_withdrawal: CryptoWithdrawal):
    if crypto_withdrawal.status == crypto_withdrawal.STATUS_COMPLETED:
        task_manager.one_off('send_email', send_email, ['Withdrawal Completed', _crypto_withdrawal_email_msg(crypto_withdrawal, ''), crypto_withdrawal.user.email])
