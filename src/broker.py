...
def _broker_order_action(db_session: Session, broker_order: BrokerOrder):
    ...
    if broker_order.status == broker_order.STATUS_READY:
        ...
        if not exchange_order_id:
            ...
            task_manager.one_off('send_email', send_email, ['failed to create exchange order', msg])
            return updated_records
    ...
    if broker_order.status == broker_order.STATUS_EXCHANGE:
        ...
        if broker_order.exchange_order:
            if dasset.order_status_check(broker_order.exchange_order.exchange_reference, broker_order.market):
                ...
                task_manager.one_off('send_email', send_email, ['failed to complete exchange order', msg])
        return updated_records
    # check expiry
...
def _broker_order_email(broker_order: BrokerOrder):
    if broker_order.status == broker_order.STATUS_FAILED:
        task_manager.one_off('send_email', send_email, ['Order Failed', _email_msg(broker_order, ''), broker_order.user.email])
    if broker_order.status == broker_order.STATUS_CANCELLED:
        task_manager.one_off('send_email', send_email, ['Order Cancelled', _email_msg(broker_order, ''), broker_order.user.email])
    if broker_order.status == broker_order.STATUS_COMPLETED:
        task_manager.one_off('send_email', send_email, ['Order Completed', _email_msg(broker_order, ''), broker_order.user.email])
    if broker_order.status == broker_order.STATUS_EXPIRED:
        task_manager.one_off('send_email', send_email, ['Order Expired', _email_msg(broker_order, ''), broker_order.user.email])
